...
| `--sort asc\|desc` | `desc` for `--state closed`, else `asc` | Sort order by sprint id (a recency proxy — higher id ≈ newer). |
| `--no-cache` | false | Bypass local cache |
| `--json` | false | NDJSON output |
| `--profile <name>` | default | Credential profile |
...
**`--all` and `--state all`.** Fetch every sprint (all states, full history) via the same single `sprintquery` call, sorted by id, with dates hydrated for **only the displayed page**. Use this when you genuinely need old sprints.

**Aggressive archive cache.** Sprints closed longer than **90 days** ago are immutable, so once their dates are hydrated they are written to a per-board archive (`sprints/<board>/archive`) with a 1-year TTL and **never refetched**. Both `--all`/date-range hydration and the default view's early-stop scan read from and populate this archive — so after the first full pass, subsequent `--all` runs only hydrate the handful of sprints that can still change (active/future/recently-closed).

**Ordering (`--sort` / closed default).** The Agile `sprint` endpoint returns sprints in a board-defined order that is *not* chronological, so any requested order requires the full set. `sprint list` sorts by **start date** (falling back to sprint id when dates are absent). `--sort` defaults to `desc` when `--state` is exactly `closed` — newest sprint first — matching the common "what ran in Q2?" question. Pass `--sort asc` for oldest-first.

**Date-range filters (`--after` / `--before`)** need every sprint's dates up front. They fetch the **complete** set via `sprintquery` + archive-backed hydration (fixing the historical under-reporting of the paged `state=closed` Agile endpoint on boards with long histories).
...
| `sprints/<boardID>/closed` | 7 days | `ListSprintsCached --state closed` page 1 (legacy path) |
