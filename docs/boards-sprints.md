...
| `--state <csv>` | _(empty → default view)_ | Sprint states to include: `active`, `future`, `closed`, or `all` |
| `--all` | false | Show every sprint (all states, full history); disables the recency window |
| `--closed-within N` | 7 | Include closed sprints whose `endDate` is within this many days (default view only; ignored with `--all` or `--after`/`--before`) |
| `--limit N` | 50 | Results per page (max 100) |
| `--page N` | 1 | Page number, 1-indexed |
...
| `--sort asc\|desc` | `desc` for `--state closed`, else `asc` | Sort order by real start date, falling back to GreenHopper sequence then sprint id when a date can't be resolved — see [Sort ordering caveats](#sprint-ordering-caveats). |
| `--no-cache` | false | Bypass local cache |
| `--json` | false | NDJSON output |
| `--profile <name>` | default | Credential profile |

**Default view (recency window).** With no `--state`, no `--all`, and no date filter, `sprint list` returns only the sprints that matter right now: **every active and future sprint, plus closed sprints that ended within the last `--closed-within` days (default 7)**. This is deliberate — the sprint endpoints are slow, and boards accumulate years of closed sprints that are rarely relevant. `--closed-within 30` widens the closed window; `--all` shows everything.

The default view is built from the GreenHopper `sprintquery` endpoint (one HTTP call returning every sprint id/name/state, cached 1 h). Closed sprints are then scanned **newest-id-first** and their dates hydrated one at a time (`GET /sprint/<id>`) only until one falls outside the window — because sprint id is a recency proxy, every older closed sprint is outside it too, so hydration **stops early**. In practice this means a couple of extra round-trips, not hundreds. The whole view is cached for 1 h (`default-<N>d`), so repeat runs make no per-sprint calls at all.

**`--all` and `--state all`.** Fetch every sprint (all states, full history) via the same single `sprintquery` call. Real dates are then resolved on the state-filtered working set (not the whole unfiltered history) before sorting — see **Ordering** below for exactly how.

**Aggressive archive cache.** Sprints closed longer than **90 days** ago are immutable, so once their dates are hydrated they are written to a per-board archive (`sprints/<board>/archive`) with a 1-year TTL and **never refetched**. `--all`/date-range hydration and the default view's early-stop scan read from and populate this archive — so after the first full pass, subsequent `--all` runs only hydrate the handful of sprints that can still change (active/future/recently-closed).

**Ordering (`--sort` / closed default).** The Agile `sprint` endpoint returns sprints in a board-defined order that is *not* chronological, so any requested order requires resolving real dates. `sprint list` sorts by **start date**. `--sort` defaults to `desc` when `--state` is exactly `closed` — newest sprint first — matching the common "what ran in Q2?" question. Pass `--sort asc` for oldest-first.

For `--all` and any explicit `--state`, dates are resolved on the state-filtered set in two bounded steps, run once per invocation right before sorting:

1. **Bulk backfill** (`BulkBackfillClosedDates`): a handful of calls to the native paged closed-sprint endpoint (`GET /board/<id>/sprint?state=closed`, up to 5 pages / 500 sprints), which returns real dates for every sprint it lists. This endpoint is *not* used as the primary source for the full closed-sprint history: it has been observed, empirically, to silently under-report on boards with long sprint histories — on the same real board referenced below it returned only 50 of 55 closed sprints, with no error or truncation signal, dropping the other 5 outright. Trusting it as a complete listing would silently corrupt totals elsewhere (`--after`/`--before`, `effort` rollups). As a bulk *date backfill* that risk doesn't apply — whatever it misses just falls through to step 2 (or, failing that, the ordering fallback below).
2. **Mop-up** (`HydrateSprintDates`): whatever's still dateless after step 1 — the bulk endpoint's own small, roughly constant blind spot, plus any non-closed sprint that genuinely has no date yet (e.g. a future sprint that hasn't started) — is resolved with a bounded-parallel per-sprint `GET /sprint/<id>` call. This step matters even though `GetSprint` is also what a "just hydrate everything individually" approach would use: because step 1 already resolves the vast majority of sprints in a small constant number of calls, the number of individual calls made here is small and does **not** scale with a board's total sprint count the way naively hydrating every sprint individually would — see [Sort ordering caveats](#sprint-ordering-caveats) for the real-board numbers this was measured against.

Both steps are best-effort: a sprint that's genuinely inaccessible (deleted, permissions) stays dateless and falls back to Sequence/ID ordering rather than erroring the whole command.

**Date-range filters (`--after` / `--before`)** need every sprint's dates up front, for the *entire* board history (not just the requested page), because the filter itself depends on dates. They intentionally skip the bulk-backfill shortcut above and fetch the **complete** set via `sprintquery` + archive-backed per-sprint hydration (`ListAllSprintsHydrated`) — the archive cache (previous paragraph) is what keeps repeat runs cheap here, not a bulk endpoint call.

### Sort ordering caveats

Two sprint identifiers are not reliable proxies for chronological order on real Jira DC instances, and `sprint list` falls back to them only when a real date genuinely cannot be resolved:

- **Sprint ID** — assigned at creation time, but board migrations, cross-project ID pools, and historical renumbering mean IDs are **not** monotonic with actual start date. Measured on a real production board (55 closed sprints, `jiracli sprint list --board 2657 --state closed`): sorting by ID alone put **41 of 55 sprints (75%)** out of true chronological order — including a 2018 sprint sandwiched between two 2017 sprints.
- **GreenHopper `sequence`** (exposed as the `sequence` field, only populated via the `sprintquery` fast path) — an instance-wide board-position/creation-order counter. It's a materially better proxy: the same measurement showed only **16 of 55 (29%)** out of order when sorting by `sequence` instead of ID. It is still an approximation, not a guarantee, so `sprint list` only uses it as a last resort when a real date isn't available.

The fallback order is therefore: **real start date** (resolved via the bulk-backfill + mop-up described above) → **`sequence`** → **sprint ID**. In practice, after bulk-backfill + mop-up, essentially every sprint sorts by its real date; `sequence`/ID only decide relative order among the rare sprints that remain genuinely dateless (e.g. two sprints compared to each other when both are inaccessible).

If you see a sprint that still looks out of place after all of this, check whether it's one of the (small, board-specific) set the paged closed-sprint endpoint doesn't return — `jiracli sprint show <id>` always reflects the true, individually-fetched date regardless of `sprint list`'s sort fallback.

Cache TTL:
- Default view → cached **1 hour** per board+window (`default-<N>d`).
- `--all` / `--state all` / ordering / `--name-contains` → sprint names cached **1 hour** per board (`names`).
- Immutable closed sprints (ended > 90 d ago) → **1 year** per board (`archive`; effectively permanent).
- Legacy `--state closed` cached-first path and `--after`/`--before` paged fallback → **7 days** per board (`closed`, `closed-all`).
- Pages 2+ reuse the same cached working set (no extra fetch).
...
{"id":2002,"name":"Sprint 43","state":"future","startDate":"2026-05-15","endDate":"2026-05-28","originBoardId":101,"sequence":2002}
```

`sequence` is present only when sourced from the GreenHopper `sprintquery` fast path (`--all`, an explicit `--state`, or the default view) and omitted (`omitempty`) otherwise — e.g. `sprint show`'s `GET /sprint/<id>` never returns it. See [Sort ordering caveats](#sprint-ordering-caveats) for what it means and when `sprint list` actually uses it.

Pagination trailer when `is_last` is false. All queries page over an in-memory
working set and report real figures:
...
`search --fields sprint` adds a Sprint line to each search result row, rendering the issue's sprint name(s) compactly — comma-separated, with the active sprint marked `(active)`, e.g. `Sprint: Sprint 41, Sprint 42 (active)`. The `sprint` token is dropped silently when the sprint field is not configured. Enable by passing `--fields sprint` (or `--fields-only key,summary,sprint`):
...
| `sprints/<boardID>/default-<N>d` | 1 hour | `sprint list` default view (recency window of N days) |
| `sprints/<boardID>/archive` | 1 year | Immutable closed sprints (ended > 90 d ago), id → hydrated record; never refetched |
| `sprints/<boardID>/names` | 1 hour | GreenHopper `sprintquery` — all sprint id/name/state in one call (`--all`, ordering, `--name-contains`, date filters) |
| `sprints/<boardID>/active+future` | 1 hour | `sprint current` / `edit sprint` (active+future resolution) |
| `sprints/<boardID>/closed` | 7 days | `ListSprintsCached --state closed` page 1 — also page 1 of `BulkBackfillClosedDates`'s bulk read (`--all` / explicit `--state` sort resolution) |
| `sprints/<boardID>/closed-all` | 7 days | `--after`/`--before` paged fallback when `sprintquery` is unavailable |

Sprint issue and board issue results are never cached. After a successful `edit sprint` move, the live sprint caches (`default-*`, `names`, `active+future`, `closed`, `closed-all`) are invalidated; the immutable `archive` is kept (a move never changes a long-closed sprint's metadata).

`BulkBackfillClosedDates`'s bulk read only caches page 1 (boards with ≤ 100 closed sprints are fully covered by it). Boards with more than 100 closed sprints re-fetch pages 2-5 on every `--all`/explicit-`--state` invocation — bounded to at most 5 page reads total regardless of cache state, so this stays cheap even uncached.
