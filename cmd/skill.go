...
// skillTargetPath returns the canonical ~/.claude/skills/jira/SKILL.md path.
...
// installSkillUnattended writes the embedded SKILL.md to its target path,
// reporting whether the install was new, an update, or already current.
// No prompts — used by `jiracli setup --install-skill`.
func installSkillUnattended(out io.Writer, content []byte) error {
...
// Used by `jiracli setup --uninstall-skill`.
